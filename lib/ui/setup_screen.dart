...
class _SetupScreenState extends State<SetupScreen> {
  late final _url = TextEditingController(
    text: widget.initial?.url ?? 'http://homeassistant.local:8123',
  );
  late final _dash = TextEditingController(
    text: widget.initial?.dashboard ?? '',
  );
  late final _name = TextEditingController(
    text: widget.initial?.name ?? 'NSPanel',
  );
  late final _mqttHost = TextEditingController(
    text: widget.initial?.mqttHost ?? '',
  );
  late final _mqttUser = TextEditingController(
    text: widget.initial?.mqttUser ?? '',
  );
  ...
  void _save() {
    ...
    if (host.isNotEmpty) {
      ...
      mqtt = {
        ...
        'password': _mqttPass.text.isEmpty
            ? (widget.initial?.mqttPass ?? '')
            : _mqttPass.text,
      };
    }
    final s = Settings(
      ...
      feedback: widget.initial?.feedback,
          ...
          children: [
            ...
            Text(
              widget.initial == null
                  ...
                  : 'New token (leave empty to keep the current one)',
            ...
            const SizedBox(height: 12),
            const Text(
              'Dashboard (url path, empty = default)',
              style: TextStyle(color: Ns.muted, fontSize: 13),
            ),
            const SizedBox(height: 6),
            TextField(
              controller: _dash,
              decoration: field.copyWith(hintText: 'e.g. nspanel'),
            ),
            const SizedBox(height: 16),
            const Text(
            ...
            TextField(
              controller: _name,
              decoration: field.copyWith(hintText: 'NSPanel Dining'),
            ),
            const SizedBox(height: 12),
            const Text(
              'MQTT broker (host or host:port, empty = no device)',
              style: TextStyle(color: Ns.muted, fontSize: 13),
            ),
            const SizedBox(height: 6),
            TextField(
              ...
              decoration: field.copyWith(
                hintText: 'e.g. 10.0.0.2 or homeassistant.local:1883',
              ),
            ),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(
                  child: TextField(
                    ...
                    decoration: field.copyWith(hintText: 'MQTT user'),
                  ),
                ),
                ...
                Expanded(
                  child: TextField(
                    ...
                    obscureText: true,
                    decoration: field.copyWith(
                      hintText: widget.initial?.mqttPass != null
                          ? 'password (kept)'
                          : 'MQTT password',
                    ),
                  ),
            ...
            SizedBox(
              ...
              child: Text(
                _err ?? '',
                style: const TextStyle(color: Ns.danger, fontSize: 13),
              ),
            ),
            const SizedBox(height: 6),
            SizedBox(
