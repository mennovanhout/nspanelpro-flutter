...
class _Broker implements MqttTransport {
  ...
  void deliver(String topic, String payload) =>
      _out.add(encodePublish(topic, payload));
...
void main() {
  ...
  setUp(() {
    ...
    bridge = PanelBridge(
      ...
      onSetting: (o, v) => commands.add('setting $o $v'),
    );
  });
  ...
  test('every entity belongs to one device and points at the same availability topic', () {
    ...
    for (final e in configs.entries) {
      ...
      expect((c['device'] as Map)['identifiers'], [
        'nspanel_abc123',
      ], reason: e.key);
      expect(
        c['availability_topic'],
        'nspanel/abc123/availability',
        reason: e.key,
      );
      expect(c['name'], isNotNull, reason: e.key);
    }
    expect(
      configs['number/abc123/brightness']!['command_topic'],
      'nspanel/abc123/brightness/set',
    );
    expect(
      configs['sensor/abc123/illuminance']!['device_class'],
      'illuminance',
    );
    expect(
      configs['binary_sensor/abc123/presence']!['device_class'],
      'occupancy',
    );
    expect(
      configs['notify/abc123/announce']!['command_topic'],
      'nspanel/abc123/say/set',
    );
    expect(
      configs['switch/abc123/screensaver']!['state_topic'],
      'nspanel/abc123/screensaver',
    );
    // the settings live on the device page's config section, with commands
    for (final o in PanelBridge.settingObjects) {
      final c = configs.values.singleWhere(
        (c) => c['unique_id'] == 'nspanel_abc123_$o',
      );
      expect(c['entity_category'], 'config', reason: o);
      expect(c['command_topic'], 'nspanel/abc123/$o/set', reason: o);
    }
    expect(
      configs['button/abc123/settings_reset']!['command_topic'],
      'nspanel/abc123/settings_reset/set',
    );
  });

  test('settings from HA arrive as object and payload, and the reset button as a press', () async {
    bridge.start();
    await pumpEventQueue();
    broker.deliver('nspanel/abc123/idle_timeout/set', '600');
    broker.deliver('nspanel/abc123/sleep/set', 'ON');
    broker.deliver('nspanel/abc123/auto_brightness/set', 'OFF');
    broker.deliver('nspanel/abc123/brightness_daylight/set', '800.0');
    broker.deliver('nspanel/abc123/settings_reset/set', 'PRESS');
    await pumpEventQueue();
    expect(commands, [
      'setting idle_timeout 600',
      'setting sleep ON',
      'setting auto_brightness OFF',
      'setting brightness_daylight 800.0',
      'setting settings_reset PRESS',
    ]);
    bridge.setting('idle_timeout', '600');
    await pumpEventQueue();
    expect(broker.published['nspanel/abc123/idle_timeout'], '600');
  });

  test(
    'on connect it announces itself: discovery, online, version, subscriptions',
    () async {
      bridge.start();
      await pumpEventQueue();
      final discovery = broker.published.keys
          .where((k) => k.startsWith('homeassistant/'))
          .toList();
      expect(discovery.length, bridge.discoveryConfigs().length);
      expect(
        jsonDecode(
          broker.published['homeassistant/sensor/abc123/proximity/config']!,
        )['name'],
        'Proximity',
      );
  ...
  test(
    'a doorbell: built-in sounds, HA paths, media browser ids, wake and volume',
    () async {
      bridge.start();
      await pumpEventQueue();
      ...
      broker.deliver(
        'nspanel/abc123/say/set',
        '{"sound": "chime", "wake": true, "volume": 80}',
      );
      broker.deliver(
        'nspanel/abc123/say/set',
        '{"message": "Someone is at the door", "wake": true}',
      );
      await pumpEventQueue();
      expect(commands, [
        'play sound:doorbell',
  ...
  test('proximity is rate-limited and presence is derived from a learned baseline', () async {
    ...
    await pumpEventQueue();
    expect(
      broker.published['nspanel/abc123/presence'],
      isNull,
      reason: 'nobody there yet',
    );
  ...
  test('the update entity: state JSON and the install command', () async {
    ...
    final cfg = jsonDecode(
      broker.published['homeassistant/update/d1/app/config']!,
    ) as Map;
    ...
    await pumpEventQueue();
    expect(
      (jsonDecode(broker.published['nspanel/d1/update']!)
          as Map)['update_percentage'],
      40,
    );
